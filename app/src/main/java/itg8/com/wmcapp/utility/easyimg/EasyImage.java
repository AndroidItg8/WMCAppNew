...
import static itg8.com.wmcapp.utility.easyimg.EasyImageFiles.singleFileList;
...
public class EasyImage implements Constants {
    ...
    private static Uri createCameraPictureFile(@NonNull Context context) throws IOException {
        ...
        Prefs.putString(KEY_PHOTO_URI, uri.toString());
        Prefs.putString(KEY_LAST_CAMERA_PHOTO, imagePath.toString());
        return uri;
    }
    ...
    private static File takenCameraPicture(Context context) throws IOException, URISyntaxException {
        String lastCameraPhoto = Prefs.getString(KEY_LAST_CAMERA_PHOTO, null);
    ...
    public static File lastlyTakenButCanceledPhoto(@NonNull Context context) {
        String filePath = Prefs.getString(KEY_LAST_CAMERA_PHOTO, null);
    ...
    private static void onPictureReturnedFromCamera(Activity activity, @NonNull Callbacks callbacks) {
        try {
            String lastImageUri = Prefs.getString(KEY_PHOTO_URI, null);
            ...
            Prefs.remove(KEY_LAST_CAMERA_PHOTO);
    ...
    public static void clearConfiguration(@NonNull Context context) {
        Prefs.remove(BundleKeys.FOLDER_NAME);
        Prefs.remove(BundleKeys.ALLOW_MULTIPLE);
        Prefs.remove(BundleKeys.COPY_TAKEN_PHOTOS);
        Prefs.remove(BundleKeys.COPY_PICKED_IMAGES);
    }
